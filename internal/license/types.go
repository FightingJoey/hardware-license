...
type License struct {
	...
	NotAfter            time.Time        `json:"notAfter"`           // UTC
...
// Payload is the plaintext inside EncryptedPayload. It re-states ID and
// NotAfter to defeat "header swap" attacks where an attacker tries to
// glue someone else's encrypted payload onto a license header.
type Payload struct {
	ID             string   `json:"id"`             // must equal License.ID
	NotAfter       time.Time `json:"notAfter"`      // must equal License.NotAfter
	Features       []string `json:"features"`
	MaxOfflineDays int      `json:"maxOfflineDays"` // 0 = unlimited
